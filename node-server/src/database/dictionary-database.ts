...
export class DictionaryDatabase {
    ...
    for (let i = 0; i < termList.length; i++) {
      ...
      for (const row of rows) {
        if (dictionaries.has(row.dictionary)) {
          ...
          const glossary = JSON.parse(row.glossary as string);
    ...
    for (let i = 0; i < kanjiList.length; i++) {
      ...
      for (const row of rows) {
        if (dictionaries.has(row.dictionary)) {
          ...
          const meanings = JSON.parse(row.meanings as string);
          const stats = row.stats ? JSON.parse(row.stats as string) : {};
  ...
  async getDictionaryInfo(): Promise<any[]> {
    ...
    return rows.map(row => {
      const counts = row.counts ? JSON.parse(row.counts as string) : {};
      return {
        title: row.title,
        version: row.version,
